...
describe('preview helpers', () => {
  ...
  it('returns occupied bench slots plus setup previews for empty slots', () => {
    ...
    const result = benchSlotsFor(player, prompt, [undefined, 0]);
  ...
  it('can preview setup cards supplied by prompt option indexes', () => {
    const player = playerView();
    const prompt = promptView();
    const snover = { name: 'Snover', fullName: 'Snover MEG', hp: 90 };
    const result = benchSlotsFor(player, prompt, [undefined, 3], [undefined, snover]);

    expect(result.map((item) => item.empty)).toEqual([false, false]);
    expect(result[1].pokemon).toEqual(snover);
  });
