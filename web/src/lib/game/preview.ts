...
export function benchSlotsFor(
  ...
  previewCards: ReadonlyArray<CardView | undefined> = benchIndexes
    .map((handIndex) => (handIndex === undefined ? undefined : player.hand[handIndex])),
): PokemonSlotView[] {
  ...
  const previews = previewCards
    .map((card, slotIndex) => {
      const slot = emptySlots.find((item) => item.index === slotIndex) ?? player.bench[slotIndex];
