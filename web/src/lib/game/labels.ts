const knownLabels: Record<string, string> = {
  ...
  WaitPrompt: 'Waiting',
  // SelectType / SelectContext enums that read better than a literal split.
  YesNo: 'Yes / No',
  IsFirst: 'Go first?',
...
// Words that should keep their casing when an identifier is sentence-cased.
const properNouns: Record<string, string> = {
  pokemon: 'Pokemon',
  hp: 'HP',
};

// PascalCase / camelCase identifier -> sentence case, e.g.
// "DiscardEnergy" -> "Discard energy", "SetupActivePokemon" -> "Setup active Pokemon".
...
export function labelFor(value: unknown): string {
  ...
  // Single PascalCase/camelCase token (CABT SelectType/SelectContext enums).
  // Sentences fall through untouched because they contain spaces/punctuation.
